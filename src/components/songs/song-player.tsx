...
import { Download, Pause, Play, Radio, Share2 } from "lucide-react"
    ...
    async function handleShare() {
        try {
            if (typeof window !== "undefined" && navigator.clipboard) {
                ...
                setMessage("Song link copied to clipboard.")
                return
            }
        } catch {
            // fall through
        }
        setMessage("Sharing coming soon.")
    }
            ...
            <div className="rounded-[1.15rem] border border-sand/10 bg-charcoal/55 p-4 sm:p-5">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
                    ...
                    <button
                        ...
                        className={`inline-flex h-12 w-full items-center justify-center gap-2 rounded-full px-5 text-sm font-black shadow-[0_16px_40px_rgba(227,122,44,0.26)] transition sm:w-auto ${
                            isThisPlaying
                ...
                <div className="mt-4">
                    <WaveformPlaceholder density="large" isActive={isThisPlaying} />
                </div>
                ...
                <div className="mt-4 flex flex-wrap gap-2.5">
                    <button
                        type="button"
                        onClick={() => setMessage("MP3 download is not connected yet.")}
                    ...
                    <button
                        type="button"
                        onClick={() => setMessage("WAV download is not connected yet.")}
                ...
                {message && (
                    <p role="status" className="mt-3 rounded-xl border border-saffron/25 bg-saffron/10 px-4 py-2.5 text-sm font-semibold text-saffron">
                        {message}
                    </p>
                )}
            </div>
